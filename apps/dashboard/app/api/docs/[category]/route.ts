...
import { getCategoryPath, logPaths } from '@/lib/paths';
...
export async function GET(
  ...
  try {
    ...
    const docPath = getCategoryPath(params.category);
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...frontMatter,
        title: frontMatter.title || params.category
      }
    });
  } catch (error) {
    ...
    return NextResponse.json(
      { 
        ...
        path: getCategoryPath(params.category)
      },
      { status: 404 }
    );
  }
